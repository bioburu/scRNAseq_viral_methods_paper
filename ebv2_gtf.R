...
library(dplyr)
#---read in fasta file
file<-read.delim('<path_to>/B_cell_EBV_tmpts_exp/ebv2.fa',
                 header = FALSE,sep = ' ')
...
gene_names<-gsub(".*[=]([^.]+)[]].*", "\\1",selected_rows$V4)
gene_names
...
gene_names<-paste0(gene_names, "-ebv2")
gene_names
#---write gene names to csv file
write.csv(gene_names,file='<path_to>/B_cell_EBV_tmpts_exp/ebv2_genes.csv')
