...
setwd('<path_to>/B_cell_EBV_tmpts_exp/SRR31408058_BcellEBV_d0_finalized_cts/outs/filtered_feature_bc_matrix')
...
data <- CreateSeuratObject(counts=matrix,
                           ...
                           min.features=200,
                           project = 'day0')
setwd('<path_to>/B_cell_EBV_tmpts_exp/SRR31408054_BcellEBV_d2_finalized_cts/outs/filtered_feature_bc_matrix')
barcodes_path <- 'barcodes.tsv.gz'
features_path <- 'features.tsv.gz'
matrix_path <- 'matrix.mtx.gz'
matrix <- ReadMtx(mtx= matrix_path, features = features_path, cells= barcodes_path)
data2 <- CreateSeuratObject(counts=matrix,
                           ...
                           min.features=200,
                           project = 'day2')
setwd('<path_to>/B_cell_EBV_tmpts_exp/SRR31408050_BcellEBV_d5_finalized_cts/outs/filtered_feature_bc_matrix')
barcodes_path <- 'barcodes.tsv.gz'
features_path <- 'features.tsv.gz'
matrix_path <- 'matrix.mtx.gz'
matrix <- ReadMtx(mtx= matrix_path, features = features_path, cells= barcodes_path)
data3 <- CreateSeuratObject(counts=matrix,
                            ...
                            min.features=200,
                            project = 'day5')
setwd('<path_to>/B_cell_EBV_tmpts_exp/SRR31408046_BcellEBV_d8_finalized_cts/outs/filtered_feature_bc_matrix')
barcodes_path <- 'barcodes.tsv.gz'
features_path <- 'features.tsv.gz'
matrix_path <- 'matrix.mtx.gz'
matrix <- ReadMtx(mtx= matrix_path, features = features_path, cells= barcodes_path)
