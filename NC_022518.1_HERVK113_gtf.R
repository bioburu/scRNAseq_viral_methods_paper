...
library(dplyr)
#---read in fasta file
file<-read.delim('<path_to>/B_cell_EBV_tmpts_exp/NC_022518.1_HERVK113.fa',
                 header = FALSE,sep = ' ')
...
name<-paste0(name, "-HERVK113")
name <- sub("_", "-", name)
name
#---write name to csv file
write.csv(name,file='<path_to>/B_cell_EBV_tmpts_exp/HERVK113_sequence.csv')
...
head(final)
write.table(data.frame(final),'<path_to>/B_cell_EBV_tmpts_exp/NC_022518.1_HERVK113.gtf',
            sep="\t",
            row.names=FALSE,
            quote = FALSE,
            col.names = FALSE)
